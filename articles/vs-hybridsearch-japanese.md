...
簡単に言うと、セマンティック検索とキーワード検索をハイブリッドに行える機能です。

具体的には、従来から Vector Search でサポートしているセマンティック検索用の密ベクトル (Dense Vector) に加えて、キーワード検索用の疎ベクトル (Sparse Vector) を今年の 5 月からサポートするようになりました。

本記事では疎ベクトルについて解説した後に Vector Search を利用した日本語によるハイブリッド検索を実装する流れを解説いたします。
...
単的に言うと、少数の要素にのみ値が入っており、その他ほとんどの要素には `0` が入っているベクトルのことです。
